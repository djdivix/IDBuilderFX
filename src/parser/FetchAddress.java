...
public static String findAddress(String txt){
	...
	String[] pattern={"linguistic","areaofinterest","declaration","address","area of interest","contact","communication","language"};
	...
	for(String s:lines)
	{
		...
		else if(s1.contains("address")||s1.contains("add."))
		{
			...
			{
				if(lines[flag].toLowerCase().contains(p)||lines[flag].length()<4)
				{
					f=1;
					break;
				}
			...
			{
				add=add+" "+lines[flag].trim();
				break;
			}
			else
			{
				add=add+" "+lines[flag].trim();
				m+=1;							//for checking previous line
				flag+=1;
			}
			}
			}
